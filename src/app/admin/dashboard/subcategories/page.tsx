...
import { Plus, Edit, Trash2, FolderTree, CheckCircle, XCircle, ArrowLeft } from 'lucide-react';
  ...
  const handleFormSubmit = async (formDataObj: FormData) => {
    ...
    try {
      const url = isEditing && editData 
        ? `/api/admin/subcategories/${editData.slug}`
      ...
      if (response.ok) {
        setSuccess(isEditing ? 'Subcategory updated successfully!' : 'Subcategory created successfully!');
        ...
        setError(data.error || (isEditing ? 'Failed to update subcategory' : 'Failed to create subcategory'));
        setFormLoading(false);
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {isEditing ? 'Edit Subcategory' : isCreating ? 'Create Subcategory' : 'Subcategories'}
          </h1>
          <p className="text-gray-600 mt-1">
            {isEditing ? 'Update subcategory information' : isCreating ? 'Add a new subcategory to organize your products' : 'Manage your product subcategories and organization'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
              <div className="bg-gradient-to-r from-green-50 to-white border-b border-gray-200 px-6 py-4">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {isEditing ? 'Update subcategory information and organization' : 'Add a new subcategory to organize your products under a parent category'}
                </p>
              </div>
              <div className="p-6 pt-6">
                <SubcategoryForm
                  subcategory={editData || undefined}
          ...
          <div className="space-y-6">
            <button
              ...
              className="w-full inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
            >
            ...
            <div className="rounded-lg bg-gradient-to-br from-green-50 to-emerald-100 border-0 shadow-sm">
              ...
              <div className="px-6 pb-6 space-y-4">
                ...
                <div className="pt-4">
                  <button
                    onClick={handleCreate}
                    className="w-full inline-flex items-center justify-center rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Add New Subcategory
                  </button>
